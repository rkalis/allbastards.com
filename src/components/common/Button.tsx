...
function Button({ label, onClick, inverted, className }: Props) {
  ...
  const fullClassName = overrideTailwindClasses(
    `${inverted ? invertedColours : regularColours} border-2 border-black shadow-sm px-4 py-2 text-xs sm:text-sm text-white font-bold focus:outline-none ${className}`,
  );
